...
class SeDGPL(nn.Module):
    ...
    # batch_arg:句子分词id，arg_mask:句子分词掩码，mask_indices:[MASK]在分词id中的位置，event_group:事件id集合
    def forward(self, mode, batch_arg, arg_mask, batch_Type_arg, mask_Type_arg, event_tokenizer_pos, event_key_pos, mask_indices, sentences, candiSet, candiLabels, batch_size, calculate_loss=True):
        ...
        for i in range(batch_size):
            for j in range(len(event_tokenizer_pos[i])):
                ...
                gate_2 = torch.sigmoid(self.W2_1(out_gate_1)).to(device)
                out_gate_2 = (torch.mul(gate_2, out_gate_1)).to(device).squeeze(0)
